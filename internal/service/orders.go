...
func (s service) ForceCheckout(ctx context.Context, payload *dto.PublicCheckout) (err error) {
	usr, err := s.repository.FindUserSessionByKey(ctx, payload.SessionKey)
	if err != nil {
		s.logger.Errorf("%s failed to authenticate user's session: %+v", logTagCheckout, err)
		return errs.ErrUnauthorized
	}
	...
	validItems := []*indto.ItemData{}
	for _, itm := range payload.Items {
		prd, err := s.repository.ForceFindProductByID(ctx, uint64(itm.ProductID))
		if err != nil {
			s.logger.Errorf("%s failed to find product for ID: %d: %+v", logTagCheckout, itm.ProductID, err)
			return errs.ErrBadRequest
		}
